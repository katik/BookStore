...
$(function(){
	...
	$(function($, headerParameters){
		...
		//提建议，并记录点击量
		$('#suggestContainer').on('click', '.js_suggest',function(){
		...
		//点击关闭
		$('div.js_global_suggest_postFeedback').on('click','.js_closePopBox', function(){
		...
		//提交建议
		$('div.js_global_suggest_postFeedback').on('click', '.js_submitSuggest',function(){
	...
	$('.js_addTipsV').on("mouseover",function(){
		showTips($(this));
	...
	//水平位置，出现在左侧
	$('.js_addTipsL').on("mouseover",function(){
		showTipsH($(this), 'left');
	}).on("mouseout",function(){
		hideTipsH($(this));
	});
	...
	//水平位置，出现在右侧
	$('.js_addTipsR').on("mouseover",function(){
